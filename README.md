...
| 文件 | 用途 |
|---|---|
| `jdbeanbot-win-1.0.3.zip` | Windows 便携版（约 54 MB） |
| `jdbeanbot-1.0.3.fpk` | 飞牛 fnOS 安装包（约 60 MB） |
| `jdbeanbot-kernel-1.0.3.tar.gz` | 飞牛内核升级包（自升级用，一般不必手动下载） |
...
```bash
# Linux / macOS
sha256sum -c jdbeanbot-win-1.0.3.zip.sha256
# Windows PowerShell
(Get-FileHash .\jdbeanbot-win-1.0.3.zip -Algorithm SHA256).Hash
# 与 .sha256 文件里的值比对
```
...
1. 把 `jdbeanbot-win-1.0.3.zip` 解压到任意目录。解压出来是一个 **`JDBeanBot` 文件夹**，
...
1. 飞牛桌面 → **应用中心** → 右上角 **「手动安装」** → 选择 `jdbeanbot-1.0.3.fpk`；
...
sudo appcenter-cli install-fpk /path/to/jdbeanbot-1.0.3.fpk -v 1

# 方式二：先准备向导参数文件，跳过交互（适合脚本化）
cat > /tmp/app-env <<'EOF'
wizard_console_password=你的控制台密码
EOF
sudo appcenter-cli install-fpk /path/to/jdbeanbot-1.0.3.fpk -e /tmp/app-env -v 1

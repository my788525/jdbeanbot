...
| 文件 | 用途 |
|---|---|
| `jdbeanbot-win-1.0.4.zip` | Windows 便携版（约 54 MB） |
| `jdbeanbot-1.0.4.fpk` | 飞牛 fnOS 安装包（约 60 MB） |
| `jdbeanbot-kernel-1.0.4.tar.gz` | 飞牛内核升级包（自升级用，一般不必手动下载） |
...
```bash
# Linux / macOS
sha256sum -c jdbeanbot-win-1.0.4.zip.sha256
# Windows PowerShell
(Get-FileHash .\jdbeanbot-win-1.0.4.zip -Algorithm SHA256).Hash
# 与 .sha256 文件里的值比对
```
...
1. 把 `jdbeanbot-win-1.0.4.zip` 解压到任意目录。解压出来是一个 **`JDBeanBot` 文件夹**，
...
3. 浏览器自动打开 <http://127.0.0.1:3000>。**首次进入请先设置一个控制台密码**（见 2.5）。
...
端口被占是最常见的起不来的原因。cmd 里查一下 3000 有没有被人用：
...
```bat
netsh advfirewall firewall add rule name="JDBeanBot" dir=in action=allow protocol=TCP localport=3000
```

然后其它设备访问 `http://<你这台电脑的IP>:3000`。
...
1. 飞牛桌面 → **应用中心** → 右上角 **「手动安装」** → 选择 `jdbeanbot-1.0.4.fpk`；
2. 按向导填写：**控制台登录密码**（其它项保持默认即可）；
3. 等待安装完成，桌面会出现「京东轻量京豆」图标；
4. 点图标进入控制台，或直接访问 `http://<NAS的IP>:3000`。
...
sudo appcenter-cli install-fpk /path/to/jdbeanbot-1.0.4.fpk -v 1

# 方式二：先准备向导参数文件，跳过交互（适合脚本化）
cat > /tmp/app-env <<'EOF'
wizard_console_password=你的控制台密码
EOF
sudo appcenter-cli install-fpk /path/to/jdbeanbot-1.0.4.fpk -e /tmp/app-env -v 1
...
> **国内网络增强（v1.0.4 起）**：实测经常出现「api.github.com 可达（检查正常）、
...
② `/api/health` 能不能打开（`http://127.0.0.1:3000/api/health`）；
③ 端口是不是被占了（Windows：`netstat -ano | findstr ":3000"`）。

...
| 文件 | 用途 |
|---|---|
| `jdbeanbot-win-1.0.0.zip` | Windows 便携版（约 54 MB） |
| `jdbeanbot-1.0.0.fpk` | 飞牛 fnOS 安装包（约 60 MB） |
| `jdbeanbot-kernel-1.0.0.tar.gz` | 飞牛内核升级包（自升级用，一般不必手动下载） |
...
```bash
# Linux / macOS
sha256sum -c jdbeanbot-win-1.0.0.zip.sha256
# Windows PowerShell
(Get-FileHash .\jdbeanbot-win-1.0.0.zip -Algorithm SHA256).Hash
# 与 .sha256 文件里的值比对
```
...
1. 把 `jdbeanbot-win-1.0.0.zip` 解压到任意目录（路径别太长，别放需要管理员权限的位置）；
2. 双击 **`start.bat`**；
...
应用中心 → 右上角「手动安装」→ 选择 `jdbeanbot-1.0.0.fpk` → 按向导填写**控制台登录密码** → 安装。
...
```bash
sudo appcenter-cli install-fpk jdbeanbot-1.0.0.fpk -v 1
# 想跳过安装向导的密码设置，可先准备一个 env 文件再 -e 传入：
# sudo appcenter-cli install-fpk jdbeanbot-1.0.0.fpk -e /tmp/app-env -v 1
